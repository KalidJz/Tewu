...
const User = {
    ...
    getLastCodUsuario: (callback) => {
    const query = 'SELECT MAX(cod_usuario) AS lastCodUsuario FROM usuario';
    db.query(query, (err, results) => {
        if (err) return callback(err);
        callback(null, results[0].lastCodUsuario || 0); // Retorna 0 si no hay registros
    });
},
